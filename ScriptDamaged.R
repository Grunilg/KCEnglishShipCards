...
write.csv(allFiles,"missingnoDamaged.csv")

####crop the images

shipinfo <- read.csv("shipinfotemp.csv")

for (i in 595:length(allFiles)) {
  CPath <- paste0("ShipsDamaged/",allFiles[i])
  
  coorx <- as.numeric(shipinfo[i,14]) + 400 - 218
  coory <- as.numeric(shipinfo[i,15]) + 400 - 200
  ...
  image_write(tmp, path = paste0("CropsShipsDamaged/",allFiles[i]), format = "png")
  print(i)
}

newnames <- list.files(path = "original cardsDamaged/", pattern = ".png")
...
for (i in 1:length(shipinfo[,1])) {
  ...
  tmpshipgirl <- image_read(paste0("CropsShipsDamaged/",allFiles[i]))
  ...
  #print
  image_write(tmpbg,paste0("ResultsDamaged/",newnames[i]))
  print(i)
}
